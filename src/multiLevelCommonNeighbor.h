...
class MultiLevelCommonNeighbor: public Attribute
{
public:
  MultiLevelCommonNeighbor(BglGraph const * const ptrGraph, 
      const int& numVertex, const int& numEdge);
  virtual ~MultiLevelCommonNeighbor ();
  /* Common Interface */
...
private:
  ...
  int  GetMaxNumCommNeghbors(const int level);
  ...
  void BFS( const BglVertex& curVertex, set<int>& vecCommNeighbors, int level, const int& levelLimit );
